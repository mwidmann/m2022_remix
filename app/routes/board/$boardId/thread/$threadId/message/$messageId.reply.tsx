...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  if (match) {
    return { success: false, code: `KHADD!`, message: `Fehler beim Speichern der Daten in der DB. Emojies verwendet? 😭` }
  }
      ...
      {!actionData || actionData.success === false ?
        <Form method="post" className="h-full max-h-96 flex flex-col py-2">
          ...
          <div className="grow-1 h-full">
            <textarea name="body" placeholder="Deine Nachricht..." defaultValue={body} className="w-full border-0 bg-transparent max-h-64 h-full"></textarea>
          </div>
          <div className="p-2 md:p-4 flex justify-between border-t border-gray-400 dark:border-gray-500">
            ...
            <div className="flex items-center justify-end space-x-2">
              ...
              {notifyByMail ? <input type="hidden" name="notification" value="1" /> : null}
              <Switch.Group>
                <div className="flex items-center justify-between">
                  <Switch.Label className={notifyByMail ? `text-gray-400` : `text-gray-700`}>
                    <svg className="h-6 w-6 mr-2" viewBox="0 0 24 24"><g fill="none"><path d="M3 8l7.89 5.26a2 2 0 0 0 2.22 0L21 8M5 19h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></g></svg>
                  ...
                  <Switch
                    ...
                    className={`${notifyByMail ? 'bg-blue-600' : 'bg-gray-200'
                  ...
                  >
                    <span
                      className={`${notifyByMail ? 'translate-x-6' : 'translate-x-1'
