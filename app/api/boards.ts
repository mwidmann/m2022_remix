...
export const fetchBoards: LoaderFunction = async ({
  request,
}): Promise<BoardsResponse | Response> => {
  try {
    ...
    const boards: Board[] = Array.from(boardsRows).map((boardsRow) => {
      ...
      if (boardsLink) {
        ...
        id = parseInt(
          boardsLink.attr('href')?.match(/brdid=(\d+)/)![1] ?? '0',
          0
        )
      } else {
      ...
      const isOpen =
        $boardsRow.find('td#norm img').attr('src')?.includes(`open.gif`) ||
        false
