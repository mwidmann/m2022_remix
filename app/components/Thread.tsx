...
export default function SingleThread({
  ...
  return (
    ...
    >
      <Link
        to={`thread/${thread.id}`}
        className="w-full text-left text-sm text-gray-900 visited:text-gray-500 neon:text-neonf-100 neon:visited:text-neonf-400 dark:text-gray-100 dark:visited:text-gray-100"
      >
        <div className="w-full p-2">
          <div className="flex justify-between">
            <p className="text-sm font-semibold md:text-base">{thread.title}</p>
          </div>
          <div className="flex w-full items-center justify-between text-xs text-gray-500 neon:text-neonf-900 dark:text-gray-300">
            <div>
