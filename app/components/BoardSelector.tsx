...
export default function BoardSelector({ boards, currentBoard }: { boards: Board[], currentBoard: number }) {
  ...
  return (
    <div className="relative">
      ...
      <button type="button" className="relative w-full bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-xs md:text-sm text-gray-500 dark:text-gray-300 lg:hidden" onClick={() => setShowList(!showList)}>
      ...
      <ul className={`absolute lg:relative z-20 mt-1 w-full lg:flex lg:space-x-4 items-end bg-white dark:bg-slate-900 lg:bg-transparent shadow-lg lg:shadow-none max-h-56 rounded-md py-1 ring-1 ring-black dark:ring-gray-300 ring-opacity-5 overflow-auto focus:outline-none text-xs md:text-sm transition-opacity duration-300 ${showList ? `opacity-100 pointer-events-auto` : `opacity-0 pointer-events-none lg:opacity-100 lg:pointer-events-auto`}`} role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-3">
        {boards.map(board => (
          <li className={
            `text-gray-900 dark:text-gray-300 lg:text-gray-100 dark:lg:text-gray-300 cursor-default select-none relative py-2 pl-3 pr-9 hover:bg-blue-200 hover:text-blue-900 lg:hover:bg-gray-700 lg:hover:text-gray-100 lg:px-3 lg:rounded-md ${board.id === currentBoard ? `lg:bg-gray-900` : ``}`
          }
